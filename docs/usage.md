...
Each result is tagged with a coarse category (stdlib or external) and a
fine-grained group -- the `#:` sections of `compat/external.txt`, e.g. `gonum`
or `grpc` -- so the matrix can be browsed per ecosystem.

The data is regenerated by hand from a dev clone (run `make compat`) and
committed to `main`; the rendered matrix is published at
