...
# Remove every generated file (the tree does not build again until `make generate`):
# cmd/extract bridges, gen_pools.go stub pools, stringer outputs, embedded zips.
clean_generate:
	@grep -rl "Code generated by cmd/extract" stdlib/core stdlib/ext 2>/dev/null | xargs rm -f
	@grep -rl "^// Code generated by gen_pools.go" internal/stubs 2>/dev/null | xargs rm -f
	rm -f vm/op_string.go symbol/kind_string.go lang/token_string.go
	rm -f stdlib/src.zip stdlib/xnet.zip stdlib/xtext.zip
